...
function _updateHearts(id, inCart) {
  document.querySelectorAll(`.prop-card__save[data-id="${id}"]`).forEach(btn => {
    ...
    btn.innerHTML = inCart
      ? '<i class="ti ti-heart-filled" style="color:var(--navy)"></i>'
...
function openDetailModal(id) {
  ...
  const featuresHTML = item.features.map((f, i) =>
    `<span style="display:inline-flex;align-items:center;gap:5px;font-size:.85rem;
      background:var(--navy-pale);border:1px solid rgba(27,58,107,.15);
      border-radius:100px;padding:3px 10px;color:var(--text-mid)">
      <i class="ti ${item.featureIcons[i] || 'ti-check'}" style="color:var(--navy)"></i>${f}
    </span>`
  ).join("");
  ...
  const photoHTML = photos
    ...
    : `<div style="height:180px;border-radius:var(--radius-md);
        background:linear-gradient(135deg,var(--navy-pale),var(--silver-light));
        display:flex;align-items:center;justify-content:center;margin-bottom:1.25rem;
        border:1px solid var(--silver-light)">
        <i class="ti ${(item.type === 'land-sale' || item.type === 'land-rent') ? 'ti-map-pin' : 'ti-building'}"
          style="font-size:3rem;color:var(--navy-light);opacity:.35"></i>
      </div>`;
    ...
    <div class="modal-inner">
      ...
      <div style="padding:2rem 2rem 0">
        ...
        <div style="display:flex;align-items:center;gap:5px;font-size:.85rem;
          ...
          <i class="ti ti-map-pin" style="color:var(--navy-light)"></i>${item.location}
        </div>
        ...
        <div style="background:var(--navy-pale);border:1px solid rgba(27,58,107,.15);
          ...
          <button onclick="toggleCart(${item.id})"
            ...
            style="display:flex;align-items:center;gap:6px;background:${isInCart(item.id) ? 'var(--navy)' : 'var(--white)'};
              color:${isInCart(item.id) ? '#fff' : 'var(--navy)'};
...
function saveMessage(data) {
  ...
  const msg = { id: Date.now(), unread: true, time: new Date().toLocaleString("en-UG"), ...data };
  messages.unshift(msg);
  try { localStorage.setItem("kp_messages", JSON.stringify(messages)); } catch(e) {}
  /* Email notification via EmailJS (configure in admin My Account) */
  sendEmailNotification(msg);
}

function sendEmailNotification(msg) {
  /* EmailJS is the lightweight no-backend email option.
     Admin configures serviceId, templateId, publicKey in My Account.
  ...
  try {
    ...
    emailjs.send(cfg.serviceId, cfg.templateId, {
      from_name:    msg.from   || "Visitor",
      from_phone:   msg.phone  || "—",
      from_email:   msg.email  || "—",
      subject:      msg.subject || "New message",
      message_body: msg.body   || "",
      msg_type:     msg.type   || "contact",
      sent_time:    msg.time,
    }, cfg.publicKey).catch(() => {/* silent — Netlify is primary channel */});
  } catch(e) { /* silent */ }
}
  ...
  modal.innerHTML = `
    ...
    <div class="edit-modal-inner">
      ...
      <h2 style="font-family:var(--font-serif);font-size:1.35rem;font-weight:700;
        ...
        <i class="ti ti-pencil" style="color:var(--navy);margin-right:8px"></i>Edit Listing #${item.id}
      </h2>
      ...
      <form id="editListingForm">
        ...
        <div style="margin-bottom:1.25rem">
          ...
          <div id="editPhotoGrid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(95px,1fr));gap:.6rem;margin-bottom:.75rem">
            ${(item.photos && item.photos.length > 0)
              ? item.photos.map((src, i) => `
                <div style="position:relative;height:85px;border-radius:6px;overflow:hidden;border:1px solid var(--silver-light);${i===0?'outline:2px solid var(--navy);':''}">
                  <img src="${src}" style="width:100%;height:100%;object-fit:cover" />
                  ${i===0 ? '<span style="position:absolute;bottom:0;left:0;right:0;background:var(--navy);color:#fff;font-size:.55rem;text-align:center;padding:2px;letter-spacing:.04em">MAIN</span>' : ''}
        ...
        <div style="display:flex;gap:1rem;align-items:center;margin-bottom:1rem">
          <label style="display:flex;align-items:center;gap:.5rem;cursor:pointer;font-size:.875rem">
            <input type="checkbox" name="available" ${item.available?"checked":""} style="accent-color:var(--navy);width:17px;height:17px" />
            Available (unchecked = Taken)
          </label>
          <label style="display:flex;align-items:center;gap:.5rem;cursor:pointer;font-size:.875rem">
            <input type="checkbox" name="featured" ${item.featured?"checked":""} style="accent-color:var(--navy);width:17px;height:17px" />
...
function _refreshEditPhotoGrid() {
  ...
  grid.innerHTML = _editPhotos.map((src, i) => `
    <div style="position:relative;height:85px;border-radius:6px;overflow:hidden;border:1px solid var(--silver-light);${i===0?'outline:2px solid var(--navy);':''}">
      <img src="${src}" style="width:100%;height:100%;object-fit:cover" />
      ${i===0 ? '<span style="position:absolute;bottom:0;left:0;right:0;background:var(--navy);color:#fff;font-size:.55rem;text-align:center;padding:2px">MAIN</span>' : ''}
      <button type="button" onclick="editRemovePhoto(${i})"
        style="position:absolute;top:3px;right:3px;background:rgba(153,27,27,.9);color:#fff;border:none;border-radius:50%;width:20px;height:20px;display:flex;align-items:center;justify-content:center;font-size:.65rem;cursor:pointer">
        <i class="ti ti-x"></i>
      </button>
...
function renderInbox() {
  ...
  if (messages.length === 0) {
    container.innerHTML = `<div class="inbox-empty">
      ...
      <span style="font-size:.8rem;color:var(--navy)">Note: messages from other devices arrive via your Netlify email notifications.</span></p>
    </div>`;
    return;
  }
  ...
  container.innerHTML = messages.map(m => `
    <div class="inbox-msg${m.unread ? " unread" : ""}" data-msgid="${m.id}">
      <div class="inbox-msg__header">
        <span class="inbox-msg__from"><i class="ti ti-user" style="color:var(--navy)"></i> ${m.from}</span>
